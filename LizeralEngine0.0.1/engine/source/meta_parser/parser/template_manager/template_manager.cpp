...
{
    std::string template_path = path + "/../template/" + template_name + ".mustache";
    m_template_pool.insert_or_assign(template_name,Utils::loadFile(template_path));
}

std::string TemplateManager::renderByTemplate(std::string template_name, Mustache::data& template_data)
{
    // std::cout << "[DEBUG] Rendering template: " << template_name << std::endl;
