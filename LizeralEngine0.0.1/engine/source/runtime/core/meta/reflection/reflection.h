...
#define LIZERAL_REFLECTION_NEW(name, ...) Reflection::ReflectionPtr(#name, new name(__VA_ARGS__));
#define LIZERAL_REFLECTION_DELETE(value) \
    if (value) \
    { \
        delete value.operator->(); \
        value.getPtrReference() = nullptr; \
    }
#define LIZERAL_REFLECTION_DEEP_COPY(type, dst_ptr, src_ptr) \
    *static_cast<type*>(dst_ptr) = *static_cast<type*>(src_ptr.getPtr());

#define TypeMetaDef(class_name, ptr) \
    Lizeral::Reflection::ReflectionInstance(Lizeral::Reflection::TypeMeta::newMetaFromName(#class_name), (class_name*)ptr)

#define TypeMetaDefPtr(class_name, ptr) \
    new Lizeral::Reflection::ReflectionInstance(Lizeral::Reflection::TypeMeta::newMetaFromName(#class_name), \
                                              (class_name*)ptr)
    ...
    namespace Reflection {
        ...
        // --- SFINAE Helper: 检测类是否有 onReflectionUpdated 成员函数 ---
        template<typename T, typename = void>
        struct has_on_reflection_updated : std::false_type {};

        template<typename T>
        struct has_on_reflection_updated<T, 
            std::void_t<decltype(std::declval<T>().onReflectionUpdated((const char*)nullptr))>> : std::true_type {};

        // 辅助函数：尝试调用通知
        template<typename T>
        void TryNotifyReflectionUpdated(T* instance, const char* field_name) {
            if constexpr (has_on_reflection_updated<T>::value) {
                instance->onReflectionUpdated(field_name);
            }
        }
