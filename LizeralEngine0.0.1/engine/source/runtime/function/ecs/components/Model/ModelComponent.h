...
#include "runtime/resource/resourceManager/resourceManager.h" // 包含 ResourceManager
    ...
    public:
        ...
        void LoadResources() {
            ...
            if (!m_ModelPath.empty()) {
                ...
                if (m_Model) {
                    // 使用静态变量，确保全局只编译一次默认 Shader，所有模型共享显存！
                    ...
                    for (auto& mesh : m_Model->GetMeshes()) {
                        ...
                        // 如果发现底层解析出来的材质是裸的（没有 Shader），立刻进行抢救
            ...
            // 2. 加载覆盖材质 (Override Materials) 的逻辑保持不变
